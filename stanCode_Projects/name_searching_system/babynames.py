...
def add_data_for_name(name_data, year, rank, name):
    ...
    if name in name_data:
        if year in name_data[name]:
            # existed_name_and_year
            if int(name_data[name][year]) >= int(rank):
                # new rank is smaller
                name_data[name][year] = rank
        # name existed but year not existed
        else:
            name_data[name][year] = rank
...
def add_file(name_data, filename):
    ...
    with open(filename, 'r') as f:
        for line in f:
            info_lst = line.split(',')
            # Save 'year' in the first line of file
            if len(info_lst) == 1:
                year = info_lst[0].strip()
                year = str(year)
            else:
                rank = info_lst[0].strip()
                name_boy = info_lst[1].strip()
                name_girl = info_lst[2].strip()
...
def read_files(filenames):
    ...
    """
    name_data = {}
    # name_data = {'aab': {'2010': '57'}, 'Nick': {'2010': '37'}}
    for i in filenames:
